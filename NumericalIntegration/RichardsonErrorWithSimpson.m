...
function retval = RichardsonErrorWithSimpson (FUNCT, a, b, error)
  ...
  matrix = [ initialStepSize CompositeSimpsonRule (FUNCT, 1, a, b) 1 ]; %{ fill the first row manually
  ...
  while matrix(size(matrix,1), 3) > error
    ...
    m = (b-a)/(2*stepSize);
    row(1,2) = CompositeSimpsonRule(FUNCT, m, a, b);
    
    %{ Fill in the richardson error
     row(1,3) = abs((row(1,2)-matrix(index-1,2)) / ((matrix(index-1,1)/row(1,1))^4 - 1));
...
%{
  ...
  2) Call this function for error estimation with richardson and composite simpson rule
    RichardsonErrorWithSimpson (f, 0, 1, 10^-7)
